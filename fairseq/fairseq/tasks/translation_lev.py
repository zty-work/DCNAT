...
class TranslationLevenshteinTask(TranslationTask):
    ...
    def inject_noise(self, target_tokens, constraints_mask = None):
        ...
        def _random_delete_wo_cs(target_tokens,constraints_mask):
            ...
            target_cutoff = (
                    constraints_len + 2
                    + (
                            (target_length - constraints_len -2)
                            * target_score.new_zeros(target_score.size(0), 1).uniform_()
                    ).long()
            )
            target_cutoff = target_score.sort(1)[1] >= target_cutoff
        ...
        if self.cfg.noise == "random_delete":
            ...
            if constraints_mask == None:
                ...
                return _random_delete_wo_cs(target_tokens,constraints_mask)
    ...
    def get_constraint_mask(self, target, constraints):
    ...
    def train_step(
        ...
        if "constraints" not in sample.keys():
            if self.constraint_training:
                sample["constraints"] = [self.constraints["train"][_.item()] for _ in sample['id']]
